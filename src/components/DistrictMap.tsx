  ...
  return (
    <div className="glass-panel" style={{ padding: '20px', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px', marginBottom: '16px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
            Схематичная карта учебного датасета: качество жизни в шести районах
          </p>
        </div>
        ...
        <div style={{ position: 'absolute', bottom: '10px', left: '12px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {decisions.map((d, i) => {
            ...
            return (
              ...
              >
                {d.measureId}: {d.districtId ? DISTRICTS[d.districtId].nameRu : 'Город'}
              </span>
            );
          })}
        </div>
      ...
      <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginTop: '8px' }}>
        Учебная модель: показатели Сарайшыка взяты из Алматы, прежний вес населения Алматы разделён поровну. Границы условные.
      </p>

...
function Sparkline({ project, value, type }: { project: number; value: number; type: 'speed' | 'delay' | 'impact' }) {
  ...
  const current = type === 'impact' ? value.toLocaleString(getLanguage(), { minimumFractionDigits: 1, maximumFractionDigits: 1 }) : number(value);
    ...
    <div className="tr-content">
      ...
      <div className="tr-workspace">
        ...
        <aside className="tr-sidebar">
          ...
          <section className="tr-panel tr-flow-panel"><div className="tr-panel-heading"><span><Route size={15} />{t("Распределение потока")}</span><span className="tr-panel-number">{t('В РЕАЛЬНОМ ВРЕМЕНИ')}</span></div><div className="tr-modal-split"><strong>{number(100 - transitShare)}<small>%</small></strong><span>{t("личный")}<br />{t("транспорт")}</span><strong className="tr-cyan">{number(transitShare)}<small>%</small></strong><span>{t("общественный")}<br />{t("транспорт")}</span></div><ChartTooltip className="chart-tooltip-target--bar" label={t('Распределение потока')} description={t('Личный транспорт: {0}%; общественный: {1}%. Доли заданы сценарием.', [number(100 - transitShare), number(transitShare)])}><div className="tr-split-bar" aria-hidden="true"><span style={{ width: `${100 - transitShare}%` }} /><span /></div></ChartTooltip><div className="tr-reroute"><GitBranch size={14} /><span>{t("На новую развязку")}</span><strong>{number(reroutedShare)}%</strong></div></section>

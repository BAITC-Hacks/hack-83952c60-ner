  ...
  return <main className="atlas" lang="ru">
    ...
    <div className={`at-workspace ${sidebar ? '' : 'at-collapsed'}`}>
      ...
      <section className="at-map-area" aria-label="Интерактивная карта Астаны">
        ...
        <div className="at-map-caption"><span className="at-eyebrow">ASTANA / SPATIAL EXPLORER</span><h2>Город в деталях.</h2><p>Реальные контуры. Новые связи.</p></div>
        ...
        <div className="at-map-bottom"><div className="at-legend">{mode === 'points' ? <><span><i className="at-dot-education" /> Образование</span><span><i className="at-dot-health" /> Медицина</span></> : <><span className="at-density-key"><i /> {cells.length ? 1 : 0}–{Math.max(0, ...cells.map(c => c.count))} объектов в ячейке</span><small>Сетка ≈ 500 м · высота: 65 м на объект</small></>}</div><div className="at-map-stats"><span><b>{number(filtered.length)}</b> объектов после фильтра</span><span><Check size={13} /> WGS 84</span><span>OSM · {date}</span></div></div>
      </section>
    </div>

...
describe('scenario library and detailed comparison', () => {
  it('computes benchmark values and ranks exact scores before newer timestamps', () => {
    ...
    expect(ranked[0].simulation.finalScore.toFixed(2)).toBe(ranked[2].simulation.finalScore.toFixed(2));
  ...
  it('compares B minus A with improving critical count and neutral budget, district and indicator details', () => {
    ...
    expect(within(score).getAllByRole('cell').map((cell) => cell.textContent)).toEqual(['53,86', '56,54', '+2,68 — улучшение']);
    ...
    expect(metrics.getByRole('row', { name: /^Средний результат города / }).textContent).toContain('−0,12');
    ...
    expect(within(screen.getByRole('region', { name: 'Сравнение оценок районов' })).getAllByRole('row')).toHaveLength(6);

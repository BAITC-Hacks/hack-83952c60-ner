...
- 52 теста модуля: сохранение населения, границы стресса, взвешивание, направления/мосты/очереди, жизненный цикл Canvas, целевой отбор и экономия API-запросов.
- TypeScript и production-сборка прошли. В общем прогоне один прежний тест сценариев превысил штатные 5 секунд; весь его файл прошёл отдельно с `--testTimeout=20000`, без изменения тестового кода.
- Демо проверено во встроенном браузере при 1280×900 и 390×844: переключение стресса 0/100, цвет заторов, очистка перегрузки и формат панели. Узкий viewport не является замером на физическом телефоне.
- В общей рабочей среде наблюдались 41–60 FPS в зависимости от фоновой нагрузки. Стабильные 60 FPS на всех устройствах не подтверждены; диагностические показатели оставлены в демо для проверки целевого оборудования.
